...
pub struct FileProtocol {
    ...
    pub close: fn(*const FileProtocol),
    pub delete: fn(*const FileProtocol),
...
pub fn io_volume(image_handle: Handle) -> *const FileIOInterface {
    ...
    // let file_handle: *const FileHandle = core::ptr::null_mut();
    // unsafe {
    //     ((*(*table).boot_services).handle_protocol)(image_handle);
    //     ((*(*table).boot_services).locate_protocol)(&guid::RAM_DISK_PROTOCOL, core::ptr::null(), &mut (file_handle as *const ()));
    // }
    ...
    unsafe {
        ...
        let res = ((*io_volume).open_volume)(io_volume, &mut file);
        if res != 0 {
            kprintln!("An error occured! {:x} OpenVolume", res);
        }
